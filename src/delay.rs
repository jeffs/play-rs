...
enum DelayState<I: Iterator, F: FnOnce() -> I> {
    ...
    // which (like any FnMut) merely borrows (rather than owning) self. See also
    // StackOverflow:
...
pub struct Delay<I: Iterator, F: FnOnce() -> I>(DelayState<I, F>);
...
pub fn delay<I: Iterator, F: FnOnce() -> I>(new: F) -> Delay<I, F> {
    Delay(DelayState::New(Some(new)))
}
